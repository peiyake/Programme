...
struct bridgeTable_entry *bridgeTable_createEntry(struct bridgeTable_entry*pdata)
{
	...
	strncpy(entry->brName,pdata->brName,strlen(pdata->brName));
	...
	entry->ifName_len = strlen(entry->ifName);
   	...
   	syslog(LOG_INFO,"Bridge[%d][%d]:%s-%s\n",entry->brID,entry->ifID,entry->brName,entry->ifName);
...
struct bridgeTable_entry * bridgeTable_getNextEntryByIndex(long brID,long ifID)
{
    struct bridgeTable_entry *ptr, *prev;
                ...
                int exact, size_t *var_len, WriteMethod ** write_method)
{
	...
	struct bridgeTable_entry *pNext = NULL;
/*
...
*/       
	...
	syslog(LOG_INFO,"Length=%d,MIB:%s\n",*length,buf);
	...
	*write_method = write_bridgeTable;
	...
	if(*length == (sizeof(bridgeTable_variables_oid)/sizeof(oid) + 2 + 1))
	{
		pdata = bridgeTable_head;
		pNext = bridgeTable_getNextEntryByIndex(pdata->brID,pdata->ifID);
		*length += 1;
		name[*length - 2] = pNext->brID;
		name[*length - 1] = pNext->ifID;
	}
	else{
		...
		if(NULL == pdata)
			return NULL;
		pNext = bridgeTable_getNextEntryByIndex(index_brid,index_ifid);
		if(NULL != pNext)
		{
			name[*length - 2] = pNext->brID;
			name[*length - 1] = pNext->ifID;
